...
export function TenderCard({ tender, index = 0 }: TenderCardProps) {
  const navigate = useNavigate();
  const { isAuthenticated } = useAuth();
  const [isFavorite, setIsFavorite] = useState(false);
  const [isLoadingFavorite, setIsLoadingFavorite] = useState(false);
  
  const mainImage = tender.productMainImage || (tender.images && tender.images[0]);
  const location = tender.deliveryLocation || "محافظة دمشق";
  const quantity = tender.quantity ? `${tender.quantity} ${tender.unit || 'كغ'}` : '500 كغ';
  const title = tender.title || tender.cropName || 'مناقصة';
  
  // Get color from API or fallback
  const apiColor = tender.productCardColor;
  ...
  useEffect(() => {
    const checkFavoriteStatus = async () => {
      ...
      try {
        ...
        const favorites = await getUserFavorites(Number(userId), 'tender');
        const isFav = favorites.some((fav) => fav.contextId === tender.tenderId);
        setIsFavorite(isFav);
      } catch (error) {
    ...
    checkFavoriteStatus();
  }, [isAuthenticated, tender.tenderId]);
  ...
  const handleFavoriteToggle = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      if (isFavorite) {
        await removeFromFavorites(userIdNum, 'tender', tender.tenderId);
        setIsFavorite(false);
      } else {
        await addToFavorites(userIdNum, 'tender', tender.tenderId);
        setIsFavorite(true);
      }
    } catch (error) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between border-b border-white/10 pb-1.5">
          <div className="flex items-center gap-1.5">
            <span className="opacity-90">الميزانية:</span>
            <span className="font-bold">{tender.maxBudget?.toLocaleString() || '-'} ل.س</span>
          </div>
          <svg className="w-3.5 h-3.5 opacity-80" fill="currentColor" viewBox="0 0 20 20">
        ...
        {tender.endTime && (
          <>
            <div className="flex items-center justify-center gap-1.5 py-1">
              ...
              <span>تاريخ انتهاء المناقصة: {formatDate(tender.endTime)}</span>
            </div>
      ...
      {tender.endTime && (
        ...
        >
          <CountdownTimer endTime={tender.endTime} />
        </div>
      )}
    </Link>
  );
