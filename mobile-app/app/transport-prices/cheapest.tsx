...
import { fetchTransportRegions, getCheapestTransportPrice } from '@/services/transport-prices';
import { useEffect, useState } from 'react';
import { Alert, ScrollView, Text, TextInput, TouchableOpacity, View } from 'react-native';

export default function CheapestPriceScreen() {
  ...
  const onCheapest = async () => {
    try {
      ...
      const res = await getCheapestTransportPrice(dto);
      const data = (res as any)?.data ?? res;
      const msg = (data && typeof data === 'object' && 'message' in data) ? (data as any).message : JSON.stringify(data);
      setResult(String(msg));
    } catch (e: any) {
      const msg = e?.response?.data?.message || e?.response?.message || e?.message || 'Failed to fetch cheapest price';
      setResult(String(msg));
    }
  };
  ...
  return (
    <ScrollView contentContainerStyle={{ padding: 16, gap: 12 }}>
      <ThemedView style={{ gap: 12 }}>
        <ThemedText type="title">Cheapest Transport Price</ThemedText>
        ...
        <TouchableOpacity onPress={onCheapest} style={{ backgroundColor: '#334155', paddingVertical: 12, borderRadius: 8, alignItems: 'center' }}>
          <Text style={{ color: 'white', fontWeight: '700' }}>Get Cheapest Price</Text>
        </TouchableOpacity>
        {!!result && (
